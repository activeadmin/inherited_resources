...
class PolymorphicFactoriesTest < ActionController::TestCase
  ...
  def test_expose_all_employees_as_instance_variable_on_index
    MyCorp::Employee.expects(:scoped).returns([mock_employee])
  ...
  def test_expose_the_requested_employee_on_show
    MyCorp::Employee.expects(:find).with('42').returns(mock_employee)
  ...
  def test_expose_a_new_employee_on_new
    MyCorp::Employee.expects(:build).returns(mock_employee)
  ...
  def test_expose_a_newly_create_employee_on_create
    MyCorp::Employee.expects(:build).with({'these' => 'params'}).returns(mock_employee(:save => true))
...
class PolymorphicCompanyTest < ActionController::TestCase
  ...
  def setup
    ...
    mock_company.expects(:employees).returns(MyCorp::Employee)
